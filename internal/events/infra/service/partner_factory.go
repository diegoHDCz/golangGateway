...
type DefaulPartnerFacotry struct {
	partnerBaseURLs map[int]string
}

func NewPartnerFactory(partnerBaseURLs map[int]string) PartnerFactory {
	return &DefaulPartnerFacotry{partnerBaseURLs: partnerBaseURLs}
}

func (f *DefaulPartnerFacotry) CreatePartner(partnerID int) (Partner, error) {
	baseURL, ok := f.partnerBaseURLs[partnerID]
	if !ok {
		return nil, fmt.Errorf("Partner ID %d not found", partnerID)
	}
	switch partnerID {
	case 1:
